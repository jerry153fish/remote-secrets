...
/// if using the local AWS emulator backend
pub fn is_test_env() -> bool {
    std::env::var("TEST_ENV").unwrap_or_default() == "true"
}

/// AWS integration tests require the local emulator environment described in the repo docs.
pub fn should_run_aws_integration_tests() -> bool {
    is_test_env()
}

/// Get the local AWS endpoint used in integration tests.
pub fn aws_endpoint_url() -> &'static str {
    let default_endpoint = "http://localhost:4566/".to_string();
